...
export default function SlotPicker({ lapanganId, selectedDate, setSelectedDate, selectedSlots, setSelectedSlots }: SlotPickerProps) {
  ...
  const toggleSlot = (jam: string) => {
    ...
    if (hasUnavailable) {
      toast.error('Gagal: Terdapat jadwal yang sudah penuh di antara jam yang Anda pilih.');
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="flex overflow-x-auto pb-4 gap-3 snap-x" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          {dates.map((date) => (
            <button
              key={date}
              type="button"
            ...
            >
              <span className="text-xs font-body font-medium uppercase mb-1">{formatDate(date).split(',')[0]}</span>
              <span className="text-xl font-bold font-display">{formatDate(date).split(',')[1].trim().split(' ')[0]}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div>
        <div className="flex justify-between items-center mb-4">
          ...
          {selectingStart && (
            <span className="text-xs font-body text-primary bg-primary/10 px-2 py-1 rounded-md animate-pulse">
              Pilih jam selesai...
            </span>
          )}
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-8 text-error text-sm font-body bg-error/10 rounded-xl border border-error/20">{error}</div>
        ) : slots.length === 0 ? (
          <div className="text-center py-8 text-text-secondary text-sm font-body bg-surface-darker rounded-xl border border-white/5">Tidak ada slot tersedia di tanggal ini.</div>
        ) : (
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
            {slots.map((slot) => {
              ...
              let btnClass = '';
              if (!slot.tersedia) {
                btnClass = 'bg-surface-darker text-text-muted border border-white/5 cursor-not-allowed opacity-50';
              } else if (isSelectingStart) {
                btnClass = 'bg-primary text-white shadow-[0px_4px_12px_rgba(249,115,22,0.3)] border border-primary ring-2 ring-primary ring-offset-2 ring-offset-surface-dark';
              } else if (isSelected) {
                btnClass = 'bg-primary text-white shadow-[0px_4px_12px_rgba(249,115,22,0.3)] border border-primary';
              } else {
                btnClass = 'bg-surface-darker text-white border border-white/10 hover:border-primary hover:text-primary hover:bg-primary/5';
              }
      ...
      <div className="flex items-center space-x-6 text-sm font-body text-text-secondary pt-2">
        <div className="flex items-center space-x-2"><div className="w-4 h-4 rounded-[4px] bg-surface-darker border border-white/10"></div><span>Tersedia</span></div>
        <div className="flex items-center space-x-2"><div className="w-4 h-4 rounded-[4px] bg-primary"></div><span className="text-white">Dipilih</span></div>
        <div className="flex items-center space-x-2"><div className="w-4 h-4 rounded-[4px] bg-surface-darker border border-white/5 opacity-50"></div><span>Penuh</span></div>
      </div>
    </div>
  );
}
